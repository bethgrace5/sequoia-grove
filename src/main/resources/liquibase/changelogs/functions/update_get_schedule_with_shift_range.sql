    ...
    from (
      ...
      s.position as pos, h.fname as mon, h.eid as mon_eid, s.pid, s.location_id, s.start_date, s.end_date
    ...
    on m_sid = su_sid
    where start_date <= to_date(suppliedMon, $token$dd-mm-yyyy$token$) and
           (end_date >= to_date(suppliedMon, $token$dd-mm-yyyy$token$) or end_date is null)
    order by index
    $BODY$
    language sql;
